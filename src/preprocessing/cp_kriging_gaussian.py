...
(Data-size dependent RMSE logging included)

Author: Yongbeen Kim (modified)
Date: 2025-09-29
"""
...
def semivariogram(x, y, n_lags=10):
    ...
    h_list, gamma_list = np.array(h_list), np.array(gamma_list)
    if h_list.size == 0:
        return np.array([]), np.array([])
    lag_edges = np.linspace(0.0, h_list.max(), n_lags+1)
...
if __name__ == "__main__":
    ...
    nugget, sill, vrange = 0.0, np.var(eta_unique), (q_unique.max() - q_unique.min()) * 0.1

    # Gaussian model 선택
    variogram_func = gaussian_variogram
    logger.info(f"Kriging model: {variogram_func.__name__}")
    # 🔹 오직 수식만 출력
    logger.info("Model formula: γ(h) = nugget + sill * (1 - exp(-(h / vrange)**2))")
    ...
    for n_pts, color in zip(points_list, colors):
        ...
        eta_pred, _ = ordinary_kriging_1d(q_spline, eta_spline, q_dense, nugget, sill, vrange, variogram_func)
    ...
    plt.title("Pump Efficiency: B-spline + Gaussian Kriging")
    plt.ylim(0, 100)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

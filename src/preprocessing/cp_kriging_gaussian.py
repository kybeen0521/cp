...
# --------------------------------------------------
# Ordinary Kriging (1D Gaussian Covariance)
# --------------------------------------------------
def ordinary_kriging_1d_gaussian(x_data, y_data, x_pred, sill=None, range_factor=0.1):
    ...
    # Gaussian covariance
    cov_matrix = np.array([[sill * np.exp(-((x_data[i]-x_data[j])/range_)**2) for j in range(n)] for i in range(n)])
...
# --------------------------------------------------
# B-spline + Kriging 예측 (10, 15, 30 pts)
# --------------------------------------------------

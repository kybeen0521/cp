...
def ordinary_kriging_1d_gaussian(x_data, y_data, x_pred, sill=None, range_factor=0.1):
    """1D Ordinary Kriging prediction with Gaussian covariance."""
    n = len(x_data)
    if sill is None:
        sill = np.var(y_data)
    range_ = (x_data.max() - x_data.min()) * range_factor

    cov_matrix = np.array([[sill * np.exp(-((x_data[i] - x_data[j])/range_)**2)
    ...
    y_pred = []
    for x in x_pred:
        k = np.array([sill * np.exp(-((x - xi)/range_)**2) for xi in x_data])
...
def plot_efficiency(Q, eta, q_unique, eta_unique, bep_Q, bep_eta, points_list, colors):
    ...
    plt.title("Pump Efficiency Curve with Gaussian Kriging Prediction (Multiple B-spline Points)")
    plt.ylim(0, 100)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    # Step 4: Plot efficiency with Gaussian Kriging prediction
    points_list = [10, 15, 30]
    colors = ["purple", "blue", "green", "orange"]
    plot_efficiency(Q, eta, q_unique, eta_unique, bep_Q, bep_eta, points_list, colors)
